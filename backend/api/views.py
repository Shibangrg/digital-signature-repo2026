...
def sign_document(request):
    ...
    try:
        ...
        if not certificate_pem:
            return Response(
                {"status": "failed", "message": "User certificate is missing."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        document_id = str(request.data.get("document_id", "")).strip() or str(uuid.uuid4())
        structured_snapshot = _canonical_json_text_from_bytes(data_bytes)
        normalized_text_for_diff, diff_kind = _normalize_text_for_diff(
        ...
        signed_package: dict[str, str] = {
            ...
            "signature_algorithm": "RSA-PKCS1v15" if algorithm == "RSA-SHA256" else "ECDSA-SECP256R1",
...
def verify_document(request):
    ...
    try:
        ...
        if not signed_package_file:
            return Response(
                {"status": "invalid", "message": "No signed package file uploaded."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            return Response(
                {"status": "invalid", "message": "File is not valid JSON."},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
def verify_stored_document(request):
    ...
    try:
        ...
        if not current_file:
            return Response({"status": "invalid", "message": "No current file uploaded."}, status=status.HTTP_400_BAD_REQUEST)
        if not document_id:
            return Response({"status": "invalid", "message": "document_id is required."}, status=status.HTTP_400_BAD_REQUEST)

        rec = DocumentRecord.objects.filter(doc_id=document_id).first()
        if not rec:
            return Response({"status": "invalid", "message": "Document ID not found."}, status=status.HTTP_404_NOT_FOUND)

        if version_no_raw:
            if not version_no_raw.isdigit():
                return Response({"status": "invalid", "message": "version_no must be numeric."}, status=status.HTTP_400_BAD_REQUEST)
            version = rec.versions.filter(version_no=int(version_no_raw)).first()
        else:
            version = rec.versions.order_by("-version_no").first()

        if not version or not hasattr(version, "artifact"):
            return Response({"status": "invalid", "message": "Stored signed artifact not found."}, status=status.HTTP_404_NOT_FOUND)
    ...
    try:
        ...
        if public_key_raw.startswith('-----BEGIN CERTIFICATE-----'):
            try:
                ...
                return Response(
                    {"status": "failed", "message": "Invalid certificate"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        else:
        ...
        try:
            ...
            return Response(
                {"status": "failed", "message": "Unsupported public key type"},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
def verify_chain(request):
    ...
    for v in versions:
        ...
        expected = _compute_chain_hash(
            v.prev_chain_hash, v.payload_hash, v.algorithm, v.signature_b64, v.created_at.isoformat()
        )
